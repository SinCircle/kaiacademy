...
function messageKindLabel(kind: DiscussionMessage["kind"]) {
  return kind === "解法" ? "进展" : kind;
}
...
function MessageThread({
  ...
  const canOpenMenu = message.canLabel || message.canHide || canAdopt || message.canDelete;
    ...
    <div className={`message-thread${expanded ? " has-children" : ""}${message.isHiddenBranch ? " hidden-branch" : ""}`} role="listitem">
      <article className="message-item">
        <div className="message-author-rail">
          ...
          {(message.isAdopted || message.kind) && <div className="message-author-indicators">
            {message.isAdopted && <BadgeCheck aria-label="已采纳" size={15} />}
            {message.kind && <span aria-label={messageKindLabel(message.kind) ?? undefined} className="message-kind-icon" title={messageKindLabel(message.kind) ?? undefined}>
        ...
        <div className="message-copy">
          <header><Link href={`/members/${message.authorId}`}>{message.authorName}</Link><time>{relativeTime(message.createdAt)}</time></header>
          ...
          <footer>
            ...
            <CommentReactions onToggle={(marker) => void action({ action: "message_reaction", messageId: message.id, marker })} reactions={message.reactions} />
        ...
        {canOpenMenu && <button className="message-more" aria-label={`更多关于 ${message.authorName} 的操作`} onClick={() => setMenuOpen((value) => !value)} type="button"><MoreHorizontal aria-hidden="true" size={15} /></button>}
        {menuOpen && canOpenMenu && <div className="message-menu">
          {message.canLabel && <><button onClick={() => { void action({ action: "message_label", messageId: message.id, kind: "解法" }); setMenuOpen(false); }} type="button"><FileCheck2 aria-hidden="true" size={14} />{message.kind === "解法" ? "取消进展" : "进展"}</button>
      ...
      {replying && <div className="inline-composer"><MessageComposer compact onSubmit={async (draft) => { const sent = await action({ action: "create_message", parentId: message.id, ...draft }); if (sent) { setReplying(false); setExpanded(true); } return sent; }} placeholder={`回复 ${message.authorName}`} /></div>}
...
export function ProblemDetailScreen({ problemId }: { problemId: string }) {
  ...
  async function load(force = false) {
    ...
    try {
      const next = force
        ...
        : await getCachedJson<ProblemDetailData>(url, { onUpdate: setData });
      setData(next);
    } catch (error) {
  ...
  async function action(payload: Record<string, unknown>) {
    ...
    if (!response.ok) { setMessage(result.message ?? "操作失败"); return false; }
    invalidateClientCache([`/api/problems/${problemId}`, "/api/problems", "/api/members/", "/api/admin/", "/api/notifications"]);
    await load(true);
    return true;
  }
      ...
      <main className="problem-detail-page">
        <article className="problem-detail-grid">
          <header className="problem-title">
            ...
            <h1><MarkdownTitle source={data.problem.title} /></h1>
