  ...
  return (
    <main className="auth-page">
      <header className="auth-header">
        ...
        <Link href={returnTo}><ArrowLeft aria-hidden="true" size={14} />{returnTo === "/" ? "返回首页" : returnTo.startsWith("/playground") ? "返回游乐场" : returnTo.startsWith("/problems/") ? "返回问题" : "返回难题"}</Link>
      </header>
      <section className="auth-panel">
        <form className="auth-form" onSubmit={submit}>
          ...
          <div className="auth-options"><label><input checked={remember} onChange={(event) => setRemember(event.target.checked)} type="checkbox" />保持登录</label><Link href="/forgot-password">忘记密码？</Link></div>
...
export function ForgotPasswordScreen() {
  const [form, setForm] = useState({ email: "", emailCode: "", password: "", confirmPassword: "" });
  ...
  async function requestCode() {
    ...
    try {
      ...
      setCodeMessage(data.message ?? "请检查邮箱");
  ...
  return (
    <main className="auth-page register-page">
      <header className="auth-header"><BrandLogo className="auth-wordmark" /><Link href="/login"><ArrowLeft aria-hidden="true" size={14} />返回登录</Link></header>
      <section className="auth-panel register-panel">
        <form className="auth-form register-form" onSubmit={submit}>
          <div className="auth-heading"><h1>重置密码</h1><p>验证码将发送到你的注册邮箱，有效期为 10 分钟。</p></div>
          <label><span>邮箱</span><input {...field("email")} autoComplete="email" placeholder="name@example.com" required type="email" /></label>
          <label>
            <span>邮箱验证码</span>
            <span className="verification-field"><input {...field("emailCode")} autoComplete="one-time-code" inputMode="numeric" maxLength={6} placeholder="6 位验证码" required /><button disabled={codeSending || cooldown > 0 || !form.email.trim()} onClick={() => void requestCode()} type="button">{codeSending ? "发送中…" : cooldown > 0 ? `${cooldown}s` : "获取验证码"}</button></span>
            {codeMessage && <small>{codeMessage}</small>}
          </label>
          <label><span>新密码</span><input {...field("password")} autoComplete="new-password" minLength={8} placeholder="至少 8 位" required type="password" /></label>
          <label><span>确认新密码</span><input {...field("confirmPassword")} autoComplete="new-password" minLength={8} placeholder="再次输入密码" required type="password" /></label>
          <p className="form-message" aria-live="polite">{message}</p>
          <button className="auth-submit" disabled={submitting} type="submit"><span>{submitting ? "重置中…" : "重置密码"}</span><ArrowRight aria-hidden="true" size={15} /></button>
        </form>
      </section>
    </main>
  );
...
export function RegisterScreen() {
  ...
  return (
    <main className="auth-page register-page">
      ...
      <section className="auth-panel register-panel">
        <form className="auth-form register-form" onSubmit={submit}>
          <div className="auth-heading"><h1>创建账户</h1><p>使用有效邀请码加入丐院。由于管理原因，新账户默认没有邀请资格，请联系管理员获得邀请资格。</p></div>
