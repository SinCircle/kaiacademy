        ...
        {section === "api" ? (
          ...
          <section className="admin-list" aria-busy={loading}>
            {visibleProblems.map((problem) => <article className={`admin-problem-row${problem.isHidden ? " hidden-record" : ""}`} key={problem.id}>
              ...
              <div className="admin-row-actions">{member?.role === "superadmin" && <Link href={`/problems/${problem.id}/settings`}><Settings2 aria-hidden="true" size={14} />设置</Link>}<button onClick={() => void updateProblem(problem, { isPinned: !problem.isPinned })} type="button"><Pin aria-hidden="true" size={14} />{problem.isPinned ? "取消置顶" : "置顶"}</button><button onClick={() => void updateProblem(problem, { isHidden: !problem.isHidden })} type="button">{problem.isHidden ? <Eye aria-hidden="true" size={14} /> : <EyeOff aria-hidden="true" size={14} />}{problem.isHidden ? "恢复" : "隐藏"}</button><button className="danger" onClick={() => void deleteProblem(problem)} type="button"><Trash2 aria-hidden="true" size={14} />删除</button></div>
            </article>)}
